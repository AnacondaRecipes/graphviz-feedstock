...
if not exist "%LIBRARY_BIN%\dot_builtins.exe" exit 1
if not exist "%LIBRARY_BIN%\dot_sandbox.exe" exit 1
...
REM Runtime DLLs (names can vary by version; check the common ones)
if not exist "%LIBRARY_BIN%\gvc.dll" exit 1
if not exist "%LIBRARY_BIN%\cgraph.dll" exit 1
if not exist "%LIBRARY_BIN%\cdt.dll" exit 1

REM ----------------------------
REM Plugin libraries exist
REM ----------------------------
if not exist "%LIBRARY_LIB%\graphviz" exit 1

REM plugins are dll on Windows
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_core.dll" exit 1
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_dot_layout.dll" exit 1
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_neato_layout.dll" exit 1
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_gd.dll" exit 1
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_pango.dll" exit 1
if not exist "%LIBRARY_LIB%\graphviz\gvplugin_webp.dll" exit 1

REM optional (depending on build options)
REM if not exist "%LIBRARY_LIB%\graphviz\gvplugin_gdk.dll" exit 1
REM if not exist "%LIBRARY_LIB%\graphviz\gvplugin_rsvg.dll" exit 1

REM config file is required so plugins load
if not exist "%LIBRARY_LIB%\graphviz\config" exit 1
if not exist "%LIBRARY_LIB%\graphviz\config6" if not exist "%LIBRARY_LIB%\graphviz\config7" if not exist "%LIBRARY_LIB%\graphviz\config8" (
  REM some builds use "config" only, but keep this to avoid false negatives
)
...
REM Render outputs
REM ----------------------------
"%LIBRARY_BIN%\dot.exe" -Tpng  -o sample.png  sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tpdf  -o sample.pdf  sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tsvg  -o sample.svg  sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tjpeg -o sample.jpeg sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tgif  -o sample.gif  sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Ttiff -o sample.tiff sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Twebp -o sample.webp sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Teps  -o sample.eps  sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tps   -o sample.ps   sample.dot || exit 1
"%LIBRARY_BIN%\dot.exe" -Tps2  -o sample.ps2  sample.dot || exit 1

"%LIBRARY_BIN%\sfdp.exe" -Tpdf -o sfdp.pdf sample.dot || exit 1

REM ----------------------------
REM Sanity: outputs are non-empty
REM ----------------------------
for %%F in (sample.png sample.pdf sample.svg sample.jpeg sample.gif sample.tiff sample.webp sample.eps sample.ps sample.ps2 sfdp.pdf) do (
  if not exist "%%F" exit 1
  for %%S in ("%%F") do if %%~zS LSS 1 exit 1
)
